...
export class UserService {
  ...
  activateToken(token: loginToken): void {
    ...
    const userJwt: loggedInUser = JSON.parse(decoded.sub);
    console.log(userJwt);

    console.log(
      `User: ${decoded.sub}, Expires At: ${new Date(decoded.exp * 1000)}`
    );
    localStorage.setItem('token', token.token);
    localStorage.setItem('expiry', new Date(decoded.exp * 1000).toISOString());

    localStorage.setItem('firstName', userJwt.firstName);
    localStorage.setItem('lastName', userJwt.lastName);
    localStorage.setItem('address', userJwt.address);
    localStorage.setItem('city', userJwt.city);
    localStorage.setItem('state', userJwt.state);
    localStorage.setItem('pin', userJwt.pin);
    localStorage.setItem('email', userJwt.email);

    this.isAuthenticated.next(true);
    this.loggedInUserInfo.next(userJwt);
    this.setAutoLogoutTimer(decoded.exp);
